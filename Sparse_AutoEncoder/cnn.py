...
def cnn_convolve(patch_dim, num_features, images, W, b, zca_white, patch_mean):
    ...
    image_channels = images.shape[2]   #一张图片的深度
    ...
    for i in range(num_images):   #遍历图片张数
        for j in range(num_features):   #遍历对应一张图片的feature map
            # convolution of image with feature matrix for each channel
            convolved_image = np.zeros(shape=(image_dim - patch_dim + 1, image_dim - patch_dim + 1),
                                       dtype=np.float64)

            for channel in range(image_channels):  #遍历一张图片的深度
